      ...
      try {
        ...
        //   } else {
             ...
             */
              ...
              if (!sender) {
                var comp = await companies.findOne({
                  attributes: ['id'],
                  where: {
                    id: senderId
                  },
                ...
                if(!comp )
                {
                    var rider = await EMPLOYEE.findOne({
                    attributes: ['id'],
                    where: {
                      id: senderId
                    },
              ...
              */
              var groupExists = await groups.findOne({
                attributes: ['id', 'name'],
                where: {
                  [Op.or]: [
                    {
              ...
              if (!groupExists) {
                ...
                groupData.createdBy = senderId;
                 groupData.orderId = data.orderId;
    ...
    */
    socket.on('leaveRoom', async (data) => {
      try {
        ...
        const groupId = data.groupId;
        if (!senderId) {
          return socket.emit('errorMessage', 'Please Provide senderId');
        }
        if (!groupId) {
    ...
    */
    socket.on('sendMessage', async (data) => {
      try {
        ...
        var orderList = await ORDERS.findOne({
          where:{
            id: groupExists.dataValues.orderId
          }
        })
        //jwt.verify(authToken, config.jwtToken, async function (err) {
          // if (err) {
          ...
          // } else {
           // const senderId = await common.userId(authToken);
            ...
            if (chatMessage) {
              ...
              var messageTitle = "You have received a new message in your order. Please check";
              //1->Customer, 2->Rider, 3->Restaurant
              // if(data.usertype == '1'){
              ...
              if(!toUser)
              {
                toUser = await companies.findOne({
                  attributes: ['deviceToken','deviceType',[sequelize.literal('companies.deviceType'), 'platform']],
                  where: {
                    id: data.receiverId
                  },
                });
              ...
              {
                senderDetails = await users.findOne({
                  attributes: ['phoneNumber'],
                  where: {
                    id: senderId
                  },
                });
                phoneNumber = senderDetails.dataValues.phoneNumber;
              }else if(data.requestStatus == '1'){
                senderDetails = await companies.findOne({
                  attributes: ['phoneNumber'],
                  where: {
                    id: senderId
                  },
                ...
                senderDetails = await EMPLOYEE.findOne({
                  attributes: ['phoneNumber'],
                  where: {
                    id: senderId
                  },
              ...
              // } 
              if(toUser){
                var orderno =orderList.dataValues.orderNo ? orderList.dataValues.orderNo : "0";
                var notifPushUserData={
                  ...
                  senderId : senderId,
                  role: data.usertype,
                  orderId: orderList.dataValues.id,
                  orderNo: "ORDER00"+orderno,
                ...
                commonNotification.sendChatNotification(notifPushUserData);
              }
   ...
   */
   ...
   socket.on('getAdmin', async (data) => {
    ...
    socket.on('updateRiderLocation', async (data) => {
      ...
      let responseNull = commonMethods.checkParameterMissing([data.empId, data.latitude, data.longitude])
      if (responseNull)
          return socket.emit('errorMessage', 'Missing Parameter required.');
      try {
        ...
        if (dataResponse) {
          ...
          //callback(null, "success")
           return socket.emit('updateRiderLocation', data);
        } else {
           return socket.emit('errorMessage', 'No Rider in list.');
        }
      } catch (err) {
